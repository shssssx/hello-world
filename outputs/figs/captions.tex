...
\textbf{Four anchors of $V$.}
(a) ground-truth $V = W_V\,\mathrm{LN}(h_{l-1,t})$.
(b) A0: per-token embed-projected anchor (no corpus, no real $V$ statistics).
(c) A1: per-token average of real $V$ over a calibration corpus -
token-determined but uses the real $W_V$.
(d) A1 plus a closed-form ridge probe captures the residual context.
(a)--(c) are diagnostic decomposition steps;
(d) is the deployable predictor used in our experiments.}
